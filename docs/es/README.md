...
└── docs/es/README.md       # Spanish workspace documentation
```
...
Before using the Wayfinder suite in a repo, run **`/setup-matt-pocock-skills`** once per repo to pick the issue tracker (GitHub by default, or local files) — it writes the *Wayfinding operations* section into the repo's `docs/en/AGENTS.md`.
